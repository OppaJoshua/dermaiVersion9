...
export default function AdminHelpdeskPage() {
  ...
  const resolveTicket = async (id: string) => {
    ...
    try {
      ...
      await supabase.from("user_support_ticket").update({ status: "closed" }).eq("ticket_id", id);
    } catch { }
  };
  ...
  const markInProgress = async (id: string) => {
    ...
    try {
      ...
      await supabase.from("user_support_ticket").update({ status: "pending" }).eq("ticket_id", id);
    } catch { }
  };
