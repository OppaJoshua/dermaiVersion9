...
export default function ClinicDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function loadAppointments() {
      ...
      const mapped: AppointmentRecord[] = ((data ?? []) as any[]).map((a: any) => {
        ...
        return {
          ...
          date: isValidDate ? apptDate.toLocaleDateString("en-PH", { month: "short", day: "numeric", year: "numeric" }) : "Schedule pending",
          time: isValidDate ? apptDate.toLocaleTimeString("en-PH", { hour: "numeric", minute: "2-digit", hour12: true }) : "—",
          notes: "",
  ...
  return (<div className="space-y-6">
    ...
    {/* ── Verification Banner ───────────────────────────────── */}
    {!loading && verificationStatus === "pending" && (<div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200">
    ...
    {!loading && verificationStatus === "rejected" && (<div className="flex items-start gap-3 p-4 rounded-xl bg-red-50 border border-red-200">
    ...
    {verificationStatus === "verified" ? (<>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Stats 2x2 */}
        <div className="lg:col-span-1 grid grid-cols-2 gap-4">
          {statCards.map((s, i) => (<motion.div key={s.label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }} className={`rounded-2xl p-4 flex flex-col justify-between min-h-[110px] relative overflow-hidden shadow-sm ${s.highlight
        ...
        {/* Line chart */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-2 bg-white border border-magenta-100 shadow-sm rounded-2xl p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {/* New patient cards */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="lg:col-span-2 bg-white border border-magenta-100 shadow-sm rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold text-gray-900">New Patients</h2>
        ...
        {/* Calendar */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="bg-white border border-magenta-100 shadow-sm rounded-2xl p-5 space-y-4">
          {/* Month nav */}
          <div className="flex items-center justify-between">
